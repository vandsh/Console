...
function Invoke-RemoteScript {
   [CmdletBinding(SupportsShouldProcess = $true, DefaultParameterSetName="InProcess")]
    ...
    if($PSCmdlet.ParameterSetName -eq "InProcess") {
        ...
        $serviceUrl += "user=" + $Username + "&password=" + $Password + "&splitOnGuid=" + $splitOnGuid
        foreach($singleConnection in $Connection) {
            $uri = $singleConnection.Uri.AbsoluteUri.TrimEnd("/") + $serviceUrl
            ...
            $Body = "$localParams <#$splitOnGuid#> $newScriptBlock"
            
            #using "application/octet-stream" because it does not even attempt to parse the response
            #$response = Invoke-RestMethod -ContentType "application/octet-stream" -Uri $uri -Method Post -Body $Body
            ...
            $response = & {
                    try {
                        ...
                        New-UsingBlock($memorystream = [IO.MemoryStream]::new([Text.Encoding]::UTF8.GetBytes($Body))) {
                            ...
                            New-UsingBlock($webStream = $webclient.OpenWrite($uri)) {
